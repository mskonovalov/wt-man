...
For an existing worktree, deletion removes both its directory and Git's worktree record. After deleting or moving a worktree, wt-man verifies that its old filesystem path no longer exists before reporting success. `missing (Git record only)` means the directory no longer exists but Git still has a stale record; deleting it removes only the record. `broken` means Git reports a prunable record while the directory still exists, usually because its `.git` link disappeared. Deleting a broken worktree explicitly removes the leftover directory before removing its Git record.
